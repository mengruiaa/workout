...
public class UserPlanActivity extends AppCompatActivity {
            ...
            switch (msg.what){
                case 1://表示接收服务端的字符串
                    ...
                    if (res.equals("null")){
                        ...
                        RecyclerView recyclerView = findViewById(R.id.rl_userPlan);
                        LinearLayoutManager layoutManager = new LinearLayoutManager(UserPlanActivity.this);
                        recyclerView.setLayoutManager(layoutManager);
                        MyPlansAdapter adapter = new MyPlansAdapter(plans,tvNote,btnAdd);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.zft_activity_user_plan);
        findViews();
        Intent intent = getIntent();
        String userName = intent.getStringExtra("userName");
        getUserPlans(ConfigUtil.SERVER_HOME + "GetUserPlans" + "?user_phone=" + userName);
    ...
    private void getUserPlans(final String s) {
        new Thread(){
            @Override
            public void run() {
                //使用网络连接，接收服务端发送的字符串
